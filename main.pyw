"""
Telegram Remote Command Bot для Windows
========================================
- При первом запуске генерирует уникальный ID и сохраняет в переменные окружения
- Каждые 60 секунд опрашивает новые сообщения
- Формат команды: id-<your_id> <command>  (например: id-abc123 dir)
- Проверяет ID бота и разрешённого отправителя
- Выполняет команды, пишет лог в log.log, отвечает результатом
